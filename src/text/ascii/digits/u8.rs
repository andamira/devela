...
impl Digits<u8> {
    ...
    pub const fn digit_at_index10_checked(self, index: u8) -> Option<u8> {
        ...
        let power = Self::POWERS10[index as usize] as u8;
    ...
    pub const fn write_digits16(self, buf: &mut [u8], offset: usize) -> usize {
        ...
        if n < 0x10 {
            ...
            buf[offset] = Ascii::DIGITS_BASE36_UPPER[n as usize];
            1
        } else {
            is![buf.len().saturating_sub(offset) < 2, return 0];
            buf[offset] = Ascii::DIGITS_BASE36_UPPER[(n >> 4) as usize];
            buf[offset + 1] = Ascii::DIGITS_BASE36_UPPER[(n & 0xF) as usize];
            2
        }
    }
    ...
    pub const fn digits16_1(self) -> u8 {
        ...
        Ascii::DIGITS_BASE36_UPPER[self.0 as usize]
    }
}
