...
macro_rules! impl_divisor {
    ...
    (@signed $t:ty | $un:ty | $up:ty | $unup:ty : $is_up:ident : $cap:literal) => {
        ...
        impl Divisor<$t> {
            ...
            #[doc = concat!["let d = Divisor::<", stringify![$t], ">::new(-21);"]]
            /// ```
            #[must_use]
            pub const fn new(d: $t) -> Option<Divisor<$t>> {
                if d == 0 {
                    ...
                    let inner = if ud.is_power_of_two() {
                        iif![d > 0; Inner::Shift(d, shift); Inner::ShiftAndNegate(d, shift)]
                    } else {
                        let (mut magic, rem) = Self::div_rem_wide_by_base(1 << (shift - 1), ud);
                        let e = ud - rem;
                        if e < 1 << shift {
                            Inner::MultiplyShift(d, d.signum() * (magic as $t + 1), shift - 1)
                        } else {
                            magic *= 2;
                            ...
                            magic += 1;
                            if d > 0 {
                                Inner::MultiplyAddShift(d, magic as $t, shift)
                            } else {
                                Inner::MultiplyAddShiftNegate(d, -(magic as $t), shift)
                            }
                        }
                    };
            ...
            pub const fn get(&self) -> $t {
                match self.inner {
                    Inner::Shift(d, _) => d,
                    Inner::ShiftAndNegate(d, _) => d,
                    Inner::MultiplyShift(d, _, _) => d,
                    Inner::MultiplyAddShift(d, _, _) => d,
                    Inner::MultiplyAddShiftNegate(d, _, _) => d,
                }
            }
            ...
            pub const fn div_of(&self, n: $t) -> $t {
                match self.inner {
                    ...
                    Inner::ShiftAndNegate(_, shift) => {
                        let mask = (1 as $t << shift).wrapping_sub(1);
                        let b = (n >> (<$t>::BITS - 1)) & mask;
                        let t = n.wrapping_add(b) >> shift;
                        t.wrapping_mul(-1)
                    },
                    Inner::MultiplyShift(_, magic, shift) => {
                        let q = Self::mulh(magic, n) >> shift;
                        iif![q < 0; q + 1; q]
                    },
                    Inner::MultiplyAddShift(_, magic, shift) => {
                        let q = Self::mulh(magic, n);
                        let t = q.wrapping_add(n) >> shift;
                        iif![t < 0; t + 1; t]
                    },
                    Inner::MultiplyAddShiftNegate(_, magic, shift) => {
                        let q = Self::mulh(magic, n);
    ...
    (@unsigned $t:ty | $up:ty : $is_up:ident : $cap:literal) => {
        ...
        impl Divisor<$t> {
            ...
            pub const fn new(d: $t) -> Option<Divisor<$t>> {
                if d == 0 {
                    ...
                    let inner = if d.is_power_of_two() {
                        ...
                        let e = d - rem;
                        if e < 1 << shift {
                            Inner::MultiplyShift(d, magic + 1, shift)
                        } else {
                            magic = magic.wrapping_mul(2);
                            let (doubled_rem, overflowed) = rem.overflowing_mul(2);
                            if doubled_rem >= d || overflowed { magic += 1; }
                            Inner::MultiplyAddShift(d, magic + 1, shift)
                        }
                    };
                    Some(Self { inner })
                }
            ...
            pub const fn div_of(&self, n: $t) -> $t {
                match self.inner {
                    Inner::Shift(_, shift) => n >> shift,
                    Inner::MultiplyShift(_, magic, shift) => Self::mulh(magic, n) >> shift,
                    Inner::MultiplyAddShift(_, magic, shift) => {
                        let q = Self::mulh(magic, n);
                        let t = ((n - q) >> 1) + q;
                        t >> shift
