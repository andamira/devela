...
// - slices: `&[i8]`, …, `&[u128]`, `&[Int<i8>]`, …, `&[Int<u128>]`.
///
...
// - Methods for `&[i32]`:
// [core][Self#fraction-related-methods-for-i32],
// `&[Int<i32>]`:
// [core][Self#fraction-related-methods-for-inti32].
//
// See also the related trait: [`NumFrac`][super::NumFrac].
#[repr(transparent)]
...
mod core_impls {
    ...
    impl<T: PartialEq> PartialEq for Frac<T> {
        #[inline] #[must_use]
        fn eq(&self, other: &Self) -> bool { self.0.eq(&other.0) }
    }
    impl<T: Eq> Eq for Frac<T> {}
    impl<T: PartialOrd> PartialOrd for Frac<T> {
        #[inline] #[must_use]
        fn partial_cmp(&self, other: &Self) -> Option<Ordering> { self.0.partial_cmp(&other.0) }
    }
    impl<T: Ord> Ord for Frac<T> {
        #[inline] #[must_use]
