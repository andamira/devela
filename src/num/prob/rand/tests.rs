...
#[test]
fn fill_bytes() {
    let mut rng = RandFake::new([0x0102_0304_0506_0708, 0xA1A2_A3A4_A5A6_A7A8]);
    let mut buf = [0u8; 11];
...
#[test]
fn bounded_helpers() {
    let mut rng = RandFake::new([27, 7, 5]);
...
fn choose_scored_picks_highest_score() {
    let mut rng = RandFake::new([10, 30, 20]);
    let data = ['a', 'b', 'c'];
