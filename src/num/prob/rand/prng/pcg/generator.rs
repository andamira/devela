...
#[doc = crate::_doc_meta!{location("num/prob/rand", macro rand_pcg)}]
///
...
macro_rules! rand_pcg {
     ...
     $SEED:literal, $MUL:literal, $INC:literal
        ...
        $crate::_devela_policy! { $vis mod [<__pcg_def_ $name >],
            ...
            devela { $crate::rand_pcg!(%define
                ...
                $crate::_ABBR_PRNG!(), "."]]
                #[doc = $crate::_doc_meta!{location("num/prob/rand", struct $name)}]
    ...
    (%define $(#[$attrs:meta])*
     ...
     $SEED:literal, $MUL:literal, $INC:literal
        ...
        impl $name {
            ...
            pub const fn [<step_ $output>](self) -> (Self, $output) {
    ...
    (%impls u8 $name:ident) => {
        ...
        impl $crate::RandTry for $name {
            ...
            fn rand_try_below(&mut self, upper: u64) -> $crate::InfallibleResult<u64> {
                debug_assert!(upper > 0);
                if upper <= u8::MAX as u64 {
                    Ok(self.next_bounded(upper as u8) as u64)
                } else {
        ...
        impl $crate::RandTry for $name {
            ...
            fn rand_try_below(&mut self, upper: u64) -> $crate::InfallibleResult<u64> {
                ...
                if upper <= u16::MAX as u64 {
                    Ok(self.next_bounded(upper as u16) as u64)
                } else {
                    let zone = u64::MAX - (u64::MAX % upper);
                    loop {
                        let v = self.next_u64();
        ...
        impl $crate::RandTry for $name {
            ...
            fn rand_try_below(&mut self, upper: u64) -> $crate::InfallibleResult<u64> {
                ...
                if upper <= u32::MAX as u64 {
                    Ok(self.next_bounded(upper as u32) as u64)
                } else {
