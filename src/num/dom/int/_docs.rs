...
_DOC_INT_FORMULA_FACTORIAL = r#"$$ n! = 1 \cdot 2 \cdot 3 \cdot \ldots \cdot (n-1) \cdot n $$"#;
_DOC_INT_FORMULA_SUBFACTORIAL_RECURSIVE = r#"$$ !n = (n - 1) * (!(n - 1) + !(n - 2)) $$"#;
_DOC_INT_FORMULA_SUBFACTORIAL_SUMMATION = r#"$$
\large !n = n! \times \sum_{k=0}^{n} \frac{(-1)^k}{k!} $$"#;
_DOC_INT_FORMULA_SUBFACTORIAL_APPROXIMATION = r#" $$
\large !n = \left\lfloor \frac{n!}{e} + \frac{1}{2} \right\rfloor =
\left\lfloor n! \times \left(\frac{1}{e}\right) + \frac{1}{2} \right\rfloor $$"#;
_DOC_INT_ALGORITHM_SUBFACTORIAL = r#"$$
- Base cases: \( !0 = 1 \) and \( !1 = 0 \).
- For \( n > 1 \), compute \( !(n - 1) \) and \( !(n - 2) \) recursively, and combine them:
  $$ \large !n = (n - 1) \cdot (!(n - 1) + !(n - 2)). $$
"#;
_DOC_INT_FORMULA_PERMUTE = r#"$$ \large P(n,r) = \frac{n!}{(n−r)!} $$"#;
_DOC_INT_FORMULA_PERMUTE_REP = r#"$$ \large P_\text{rep}(n,r) = n_r $$"#;
_DOC_INT_FORMULA_COMBINE = r#"$$ \large C(n,r) = {n \choose r} = \frac{n!}{(n−r)!r!} $$"#;
_DOC_INT_FORMULA_COMBINE_REP = r#"$$
\large C(n+r-1,r) = {n+k-1 \choose r} = \frac{(n+r-1)!}{(n−1)!r!} $$"#;

/* division */

_DOC_INT_NOTATION_DIV_CEIL = r#"$$ \large \left\lceil \frac{x}{y} \right\rceil $$"#;
_DOC_INT_FORMULA_DIV_CEIL = r#"$$
\large
\text{div\\_ceil}(a, b) =
\begin{cases}
...
\end{cases}
$$"#;
_DOC_INT_NOTATION_DIV_FLOOR = r#"$$ \large \left\lfloor \frac{x}{y} \right\rfloor $$"#;

/* primes */

_DOC_INT_NOTATION_PI = r#"$$ \pi(x) $$"#;
_DOC_INT_ALGORITHM_TOTIENT = r#"
...
\end{cases}
$$"#;
_DOC_INT_ALGORITHM_SQRT_CEIL = r#"$$
\large
\begin{align}
...
\end{align}
$$"#;
_DOC_INT_ALGORITHM_SQRT_FLOOR = r#"
...
"#;
_DOC_INT_ALGORITHM_SQRT_ROUND = r#"$$
\begin{align}
...
_DOC_INT_FORMULA_ROOT_CEIL_SIGNED = r#"$$
\large \left\lceil |a|^{\frac{1}{n}} \right\rceil \cdot \text{sign}(a) = m $$"#;
_DOC_INT_PIECEWISE_ROOT_CEIL_SIGNED = r#"$$
\large
\begin{align}
...
\end{align}
$$"#;
_DOC_INT_ALGORITHM_ROOT_CEIL_SIGNED = r#"
...
_DOC_INT_FORMULA_ROOT_CEIL_UNSIGNED = r#"$$
\large \left\lceil a^{\frac{1}{n}} \right\rceil = m $$"#;
_DOC_INT_PIECEWISE_ROOT_CEIL_UNSIGNED = r#" $$
\large
\begin{align}
\notag \text{If } n = 0, \text{ then error.} \cr
\notag \text{If } n = 1, \text{ then output } a. \cr
...
\end{align}
$$"#;
_DOC_INT_ALGORITHM_ROOT_CEIL_UNSIGNED = r#"
The algorithm computes the smallest integer $ m $ such that:
$$ \large m^n \geq a. $$
...
_DOC_INT_FORMULA_ROOT_FLOOR_SIGNED = r#"$$
\large \left\lfloor |a|^{\frac{1}{n}} \right\rfloor = m \cdot \text{sign}(a) $$"#;
_DOC_INT_PIECEWISE_ROOT_FLOOR_SIGNED = r#"$$
\large
\begin{align}
\notag \text{If } n = 0, \text{ then error.} \cr
\notag \text{If } n = 1, \text{ then output } a. \cr
...
\end{align}
$$"#;
_DOC_INT_ALGORITHM_ROOT_FLOOR_SIGNED = r#"
...
_DOC_INT_FORMULA_ROOT_FLOOR_UNSIGNED = r#"$$
\large \left\lfloor a^{\frac{1}{n}} \right\rfloor = m $$"#;
_DOC_INT_PIECEWISE_ROOT_FLOOR_UNSIGNED = r#"$$
\large
\begin{align}
\notag \text{If } n = 0, \text{ then error.} \cr
\notag \text{If } n = 1, \text{ then output } a. \cr
...
\end{align}
$$"#;
_DOC_INT_ALGORITHM_ROOT_FLOOR_UNSIGNED = r#"
The algorithm computes the floored nth root,
