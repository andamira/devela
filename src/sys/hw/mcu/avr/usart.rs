...
use crate::{__cfg_item_unsafe_show, AvrReg8, is, macro_apply};
...
impl AvrUsart {
    ...
    const UDRE: u8 = 1 << 5; // Data-register empty.
    const U2X: u8 = 1 << 1; // Double asynchronous transmission speed.

    // UCSRnB
    const RXEN: u8 = 1 << 4; // Receiver enable.
    const TXEN: u8 = 1 << 3; // Transmitter enable.
    const UCSZ2: u8 = 1 << 2; // Character-size bit 2.

    // UCSRnC
    const UCSZ1: u8 = 1 << 2; // Character-size bit 1.
    const UCSZ0: u8 = 1 << 1; // Character-size bit 0.
