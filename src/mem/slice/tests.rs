...
fn range() {
    ...
    assert_eq!(Slice::range(&[1, 2, 3], 0, 0), &[][..]);
    assert_eq!(Slice::range_checked(&[1, 2, 3], 1, 3), Some(&[2, 3][..]));
    assert_eq!(Slice::range_checked(&[1, 2, 3], 0, 0), Some(&[][..]));
    ...
    assert_eq!(Slice::range_mut(&mut [1, 2, 3], 0, 0), &mut [][..]);
    assert_eq!(Slice::range_mut_checked(&mut [1, 2, 3], 1, 3), Some(&mut [2, 3][..]));
    assert_eq!(Slice::range_mut_checked(&mut [1, 2, 3], 0, 0), Some(&mut [][..]));
