// devela/src/media/audio/pcm/tests.rs
...
const PCM_SPEC_48: PcmSpec = PcmSpec::new(PcmSample::I16, AudioChannels::Stereo, 48_000);
const PCM_SPEC_41: PcmSpec = PcmSpec::new(PcmSample::I16, AudioChannels::Stereo, 41_000);
const PCM_INTERLEAVED: PcmBuf<i16, &[i16]> = PcmBuf::new(&[0, 1, 2, 3], PCM_SPEC_48);
...
const PCM_PLANAR: PcmBuf<i16, &[&[i16]]> = PcmBuf::new(PLANES, PCM_SPEC_48);
...
fn pcm_buf_interleaved_mut_slice() {
    let mut data = [0i16; 4];
    let mut pcm = PcmBuf::new(&mut data[..], PCM_SPEC_41);
...
fn pcm_buf_planar_metadata_and_frames() {
    ...
    let pcm = PcmBuf::new(planes, PCM_SPEC_48);
    assert_eq![pcm.sample(), PcmSample::I16];
    assert_eq![pcm.channels(), AudioChannels::Stereo];
    assert_eq![pcm.channel_count(), 2];
    assert_eq![pcm.sample_rate(), 48_000];
...
fn pcm_buf_planar_rejects_uneven_planes() {
    ...
    let right = [5i16, 6, 7];
    let planes: &[&[i16]] = &[&left, &right];
    let pcm: PcmBuf<i16, &[&[i16]]> = PcmBuf::new(planes, PCM_SPEC_48);
    assert_eq![pcm.frames(), None];
    assert!(!pcm.has_complete_frames());
}
#[test]
fn pcm_buf_planar_mut_slice() {
    ...
    let mut pcm = PcmBuf::new(&mut planes[..], PCM_SPEC_41);
    assert_eq![pcm.planes_len(), 2];
    assert_eq![pcm.frames(), Some(2)];
...
fn pcm_buf_planar_visit_each_mut() {
    ...
    let mut right = [3i16, 4];
    let mut planes: [&mut [i16]; 2] = [&mut left, &mut right];
    let mut pcm: PcmBuf<i16, &mut [&mut [i16]]> = PcmBuf::new(&mut planes[..], PCM_SPEC_41);
