...
impl SixelChar {
    ...
    pub const fn from_hexagram(hexagram: char) -> Option<Self> {
        ...
        if h >= 0x4DC0 && h <= 0x4DFF {
            Some(Self::from_bitmask(Self::CHAR_HEXAGRAM_TO_SIXEL[h - 0x4DC0]))
        } else {
            None
        }
    }
    ...
    pub const fn to_hexagram(self) -> char {
        Self::SIXEL_TO_CHAR_HEXAGRAM[self.as_bitmask() as usize]
    }
    ...
    pub const fn to_hexagram_inverted(self) -> char {
        Self::SIXEL_TO_CHAR_HEXAGRAM[(!self.as_bitmask() & Self::MASK) as usize]
    }
...
impl SixelChar {
    ...
    const _SIXEL_HEXAGRAM_CORRESPONDANCE: () = {
        let mut index = 0;
        while index < 64 {
            let bitmask = Self::CHAR_HEXAGRAM_TO_SIXEL[index];
            let char_from_bitmask = Self::SIXEL_TO_CHAR_HEXAGRAM[bitmask as usize];
            ...
            let roundtrip_bitmask = Self::CHAR_HEXAGRAM_TO_SIXEL[index_from_char];
