...
fn view_flags_control_visibility() {
    let view = UiView::new(UiId::new(8), LayoutRect::default()).with_flag(UiViewFlags::HIDDEN);
    assert!(view.is_hidden());
    assert!(!view.is_visible());
...
fn view_layer_can_be_changed() {
    let view = UiView::new(UiId::new(9), LayoutRect::default()).with_layer(UiLayer::new(4));
