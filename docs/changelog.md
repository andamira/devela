...
#### data::topol::ord
- new types: `Concat`, `SeqNext`, `SeqNode`, `SeqPrevNext`.
...
- new root module.
- update `define_error!`:
  - make error tags explicit rather than automatic.
...
- rename `Vector2d` and `Vector3d` to `Vector2` and `Vector3`, respectively.
