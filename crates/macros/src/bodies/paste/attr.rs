// devela_macros::bodies::paste::attr
//
//! Special `#[doc = …]`/nested attribute handling.
//

use super::{Paste, PasteResult, PasteSegment};
use core::{iter, mem, str::FromStr};
...
impl Paste {
    pub fn expand_attr(
        ...
        let group = loop {
            ...
            match token {
                ...
                {
                    let mut count = 0;
                    if tokens.inspect(|_| count += 1).all(|tt| Paste::is_attr_stringlike(&tt))
                        && count > 1
                    {
                        *contains_paste = true;
                        let leading = leading_colons + leading_path;
                        return Paste::do_paste_name_value_attr(attr, span, leading);
                    }
                    return Ok(attr);
                }
                // parens after `$(::)? $($ident)::+`
        ...
        for tt in group.stream() {
            match &tt {
                TokenTree::Punct(punct) if punct.as_char() == ',' => {
                    expanded.extend(Paste::expand_attr(
                        nested_attr,
                        group.span(),
    ...
    fn do_paste_name_value_attr(
        ...
        let mut lit = PasteSegment::paste(&segments)?;
        ...
        let mut lit = TokenStream::from_str(&lit).unwrap().into_iter().next().unwrap();
        lit.set_span(span);
        expanded.extend(iter::once(lit));
        Ok(expanded)
    }

    fn is_attr_stringlike(token: &TokenTree) -> bool {
        match token {
            ...
            TokenTree::Group(group) => {
                ...
                match inner.next() {
                    Some(first) => inner.next().is_none() && Paste::is_attr_stringlike(&first),
                    None => false,
                }
            }
