//
//! Defines [`__Divisor`], [`__divisor!`].
//

#[doc(hidden)]
/// Inner representation of a type defined with [`divisor`].
#[doc = crate::_doc_meta!{
    location("num/dom/int", struct __Divisor),
}]
#[derive(Clone, Copy)]
pub enum __Divisor<T> {
    Shift(T, u8),
...
/// Private API of [`divisor!`][crate::divisor].
#[doc(hidden)]
#[macro_export]
macro_rules! __divisor· {
    // $Type: the full type
    // $Name: just the name
    (
    /* private API */
    // supported representations
     %impl $Type:ty, $Name:ident; u8) => {
        $crate::__divisor!(%impl_u $Type,$Name; u8|u16:Y); };
    (%impl $Type:ty,$Name:ident; u16) => {
        $crate::__divisor!(%impl_u $Type,$Name; u16|u32:Y); };
    (%impl $Type:ty,$Name:ident; u32) => {
        $crate::__divisor!(%impl_u $Type,$Name; u32|u64:Y); };
    (%impl $Type:ty,$Name:ident; u64) => {
        $crate::__divisor!(%impl_u $Type,$Name; u64|u128:PW); };
    (%impl $Type:ty,$Name:ident; u128) => {
        $crate::__divisor!(%impl_u $Type,$Name; u128|u128:N); };
    (%impl $Type:ty,$Name:ident; usize) => {
        $crate::__divisor!(%impl_u $Type,$Name; usize|$crate::usize_up:Y); };
    (%impl $Type:ty,$Name:ident; i8) => {
        $crate::__divisor!(%impl_i $Type,$Name; i8|u8|i16|u16:Y); };
    (%impl $Type:ty,$Name:ident; i16) => {
        $crate::__divisor!(%impl_i $Type,$Name; i16|u16|i32|u32:Y); };
    (%impl $Type:ty,$Name:ident; i32) => {
        $crate::__divisor!(%impl_i $Type,$Name; i32|u32|i64|u64:Y); };
    (%impl $Type:ty,$Name:ident; i64) => {
        $crate::__divisor!(%impl_i $Type,$Name; i64|u64|i128|u128:PW); };
    (%impl $Type:ty,$Name:ident; i128) => {
        $crate::__divisor!(%impl_i $Type,$Name; i128|u128|i128|u128:N); };
    (%impl $Type:ty,$Name:ident; isize) => {
        $crate::__divisor!(%impl_i $Type,$Name;
    ...
    (// specific implementations
     %impl_i $Type:ty, $Name:ident; $t:ty | $un:ty | $up:ty | $unup:ty : $is_up:ident) => {
        $crate::__divisor![%traits $Type, $Name; $t]; // utility traits

        impl $Type {
            $crate::__divisor![%shared $Type,$Name; $t|$un|$up|$unup:$is_up]; // shared methods
            ...
            pub const fn new(d: $t) -> Option<$Type> {
                if d == 0 {
                    ...
                    let inner = if ud.is_power_of_two() {
                        if d > 0 { $crate::__Divisor::Shift(d, shift) }
                        else { $crate::__Divisor::ShiftAndNegate(d, shift) }
                    } else {
                        let (mut magic, rem) = Self::div_rem_wide_by_base(1 << (shift - 1), ud);
                        let e = ud - rem;
                        if e < 1 << shift {
                            $crate::__Divisor::MultiplyShift(d, d.signum()
                            ...
                            magic += 1;
                            if d > 0 {
                                $crate::__Divisor::MultiplyAddShift(d, magic as $t, shift)
                            } else {
                                $crate::__Divisor::MultiplyAddShiftNegate(d, -(magic as $t), shift)
                            }
                        }
                    };
                    Some(Self { inner })
            ...
            pub const fn get(&self) -> $t {
                match self.inner {
                    $crate::__Divisor::Shift(d, _) => d,
                    $crate::__Divisor::ShiftAndNegate(d, _) => d,
                    $crate::__Divisor::MultiplyShift(d, _, _) => d,
                    $crate::__Divisor::MultiplyAddShift(d, _, _) => d,
                    $crate::__Divisor::MultiplyAddShiftNegate(d, _, _) => d,
                }
            }
            ...
            pub const fn div_of(&self, n: $t) -> $t {
                match self.inner {
                    $crate::__Divisor::Shift(_, shift) => {
                        let mask = (1 as $t << shift).wrapping_sub(1);
                        let b = (n >> (<$t>::BITS - 1)) & mask;
                        n.wrapping_add(b) >> shift
                    },
                    $crate::__Divisor::ShiftAndNegate(_, shift) => {
                        let mask = (1 as $t << shift).wrapping_sub(1);
                        let b = (n >> (<$t>::BITS - 1)) & mask;
                        let t = n.wrapping_add(b) >> shift;
                        t.wrapping_mul(-1)
                    },
                    $crate::__Divisor::MultiplyShift(_, magic, shift) => {
                        let q = Self::mulh(magic, n) >> shift;
                        $crate::is![q < 0, q + 1, q]
                    },
                    $crate::__Divisor::MultiplyAddShift(_, magic, shift) => {
                        let q = Self::mulh(magic, n);
                        let t = q.wrapping_add(n) >> shift;
                        $crate::is![t < 0, t + 1, t]
                    },
                    $crate::__Divisor::MultiplyAddShiftNegate(_, magic, shift) => {
                        let q = Self::mulh(magic, n);
    ...
    (%impl_u $Type:ty, $Name:ident; $t:ty | $up:ty : $is_up:ident) => {
        ...
        impl $Type {
            $crate::__divisor![%shared $Type, $Name; $t|$t|$up|$up:$is_up]; // shared methods
            ...
            pub const fn new(d: $t) -> Option<$Type> {
                if d == 0 {
                    ...
                    let inner = if d.is_power_of_two() {
                        $crate::__Divisor::Shift(d, shift)
                    } else {
                        let (mut magic, rem) = Self::div_rem_wide_by_base(1 << shift, d);
                        let e = d - rem;
                        if e < 1 << shift {
                            $crate::__Divisor::MultiplyShift(d, magic + 1, shift)
                        } else {
                            magic = magic.wrapping_mul(2);
                            let (doubled_rem, overflowed) = rem.overflowing_mul(2);
                            if doubled_rem >= d || overflowed { magic += 1; }
                            $crate::__Divisor::MultiplyAddShift(d, magic + 1, shift)
                        }
                    };
                    Some(Self { inner })
                }
            ...
            pub const fn div_of(&self, n: $t) -> $t {
                match self.inner {
                    $crate::__Divisor::Shift(_, shift) => n >> shift,
                    $crate::__Divisor::MultiplyShift(_, magic, shift)
    ...
    // ($($tt:tt)*) => {
    //     compile_error![ concat!("Invalid input for `divisor!`:\n\t", // MAYBE
    //     stringify!($($tt)*))];
    // };
}
#[doc(hidden)]
pub use __divisor· as __divisor;
