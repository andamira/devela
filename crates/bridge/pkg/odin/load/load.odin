...
library_path :: proc() -> string {
    when ODIN_OS == .Linux {
        return "../../libs/libdevela_bridge.so"
    } else when ODIN_OS == .Darwin {
        return "../../libs/libdevela_bridge.dylib"
    } else when ODIN_OS == .Windows {
        return "../../libs/devela_bridge.dll"
    } else {
        return ""
    }
}
...
load :: proc() -> (lib: Library, ok: bool) {
    ...
    if !ok1 {
        fmt.eprintln("devela_bridge: failed to load library: ", path)
        return {}, false
    }
    ...
    if !ok {
        ...
        fmt.eprintln("devela_bridge: failed to load one or more symbols")
        return {}, false
    }
