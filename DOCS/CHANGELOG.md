...
- new types:
  ...
  - lang: `g_*`, `js_*`, `JsEvent`, `JsInstant`, `JsPermission`, `JsPermissionState`, `JsTextMetrics`, `JsTextMetricsFull`, `JsTimeout`, `JsWorker`, `JsWorkerError`, `JsWorkerJob`.
...
- change features into cfg flags:
  - `nightly`, `nightly_autodiff`, `nightly_bigint`, `nightly_coro`, `nightly_simd`, `nightly_stable`, `nightly_stable_next1`, `nightly_stable_next2`, `nightly_stable_later`.
- rename flags:
