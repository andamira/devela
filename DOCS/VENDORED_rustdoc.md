<!-- Updated intra-doc links, to be loaded from: _doc::vendored -->

<!-- orig: Apache-2.0 OR MIT -->
  [`cfg-if`]: crate::code::util::cfg_if
  [`ConstList`]: crate::data::list::ConstList
  [M_crossbeam-utils]: crate::_info::vendored::modifications#crossbeam-utils
  [`CacheAlign`]: crate::sys::mem::CacheAlign
  [M_etcetera]: crate::_info::vendored::modifications#etcetera
  [`AppEnv`]: crate::sys::env::AppEnv
  [`fmt_or`]: crate::code::result::ExtOption#fmt_or
  [`fmt_or_else`]: crate::code::result::ExtOption#fmt_or_else
  [`fmt_or_empty`]: crate::code::result::ExtOption#fmt_or_empty
  [M_fxhash]: crate::_info::vendored::modifications#fxhash
  [`HasherFx`]: crate::data::codec::hash::HasherFx
  [M_no_std_io]: crate::_info::vendored::modifications#no_std_io
  [`io`]: crate::sys::io
  [M_numtoa]: crate::_info::vendored::modifications#numtoa
  [`NumToStr`]: crate::text::fmt::NumToStr
  [M_pollster]: crate::_info::vendored::modifications#pollster
  [`block_on`]: crate::work::future::ExtFuture::block_on
  [M_quickdiv]: crate::_info::vendored::modifications#quickdiv
  [`Divisor`]: crate::num::Divisor
  [M_stack_dst]: crate::_info::vendored::modifications#stack_dst
  [`data::dst`]: crate::data::dst
  [M_stated-scope-guard]: crate::_info::vendored::modifications#stated-scope-guard
  [`ScopeGuard`]: crate::code::ScopeGuard
  [M_static_assertions]: crate::_info::vendored::modifications#static_assertions
  [`const_assert`]: crate::code::util::const_assert
  [`ConstBool`]: crate::num::logic::ConstBool
  [M_tailcall-chunk]: crate::_info::vendored::modifications#tailcall-chunk
  [`VecChunk`]: crate::data::list::array::VecChunk
  [M_tupl]: crate::_info::vendored::modifications#tupl
  [`Tuple`]: crate::data::list::tuple::Tuple

<!-- orig: MIT -->
  [`cfor`]: crate::code::util::cfor
  [M_crunchy]: crate::_info::vendored::modifications#crunchy
  [`unroll`]: crate::code::util::unroll
  [M_icy_sixel]: crate::_info::vendored::modifications#icy_sixel
  [`media::image::sixel`]: crate::media::image::sixel
  [M_object-id]: crate::_info::vendored::modifications#object-id
  [`IdPin`]: crate::data::uid::IdPin
  [`IdPinBox`]: crate::IdPinBox
  [`as_bytes`]: crate::sys::mem::Mem::as_bytes
  [`as_bytes_mut`]: crate::sys::mem::Mem::as_bytes_mut
  [`FatPtr`]: crate::sys::mem::FatPtr

<!-- orig: OTHER -->
  [`Xyza8a`]: crate::num::rand::Xyza8a
  [`Xyza8b`]: crate::num::rand::Xyza8b
  [`Chain`]: crate::code::result::Chain
  [`Hook`]: crate::code::result::Hook
  [M_blitfonts]: crate::_info::vendored::modifications#blit-fonts
  [`FONT_3_5`]: crate::media::font::FONT_3_5
  [`FONT_5_6`]: crate::media::font::FONT_5_6
  [M_pengy]: crate::_info::vendored::modifications#pengy
  [`HasherPengy`]: crate::data::codec::hash::HasherPengy
  [`size_of_expr`]: crate::sys::mem::size_of_expr
  [`Xabc`]: crate::num::rand::Xabc
